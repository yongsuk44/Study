...
> 이 시점에서 생기는 타당한 질문은 "컴포즈 팀은 원하는 동작을 구현하기 위해, 왜 suspend를 사용하지 않았는가?" 입니다.
...
> **컴포즈의 목표는 런타임에서 다양한 방식으로 최적화할 수 있는 큰 호출 그래프의 인메모리 표현을 만드는 것이기에 `Continuation`과의 사용 사례 자체가 다릅니다.**
> 컴포즈는 호출 컨텍스트를 강제하는 것만 필요로 합니다. 
...
그리고 컴포즈 런타임은 이 사실에 의존합니다. 이 점이 컴포즈 컴파일러가 컴포저블 함수를 다른 컴포저블 함수에서만 호출되도록 강제하는 이유입니다.
...
컴포즈의 사용방식은 오직 컴포저블 함수들로만 호출 그래프를 구성하는 것이기에 위와 같은 문제를 걱정하지 않아도 됩니다.
