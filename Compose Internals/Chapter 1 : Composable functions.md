...
컴포저블 함수는 Compose의 원자 빌딩 블록(atomic building block)이자, 컴포저블 트리를 작성하는데 사용되는 구조 입니다.  
트리로 표현하는 이유는 Compose 런타임이 큰 트리의 노드로 컴포저블 함수를 메모리에 표현할 수 있기 때문입니다. 
...
이와 같은 작업은 Compose 용어로 '발행(emitting)'이라고 하며, 발행은 노드 트리에 변경 사항을 추가하는 것을 말합니다.  
...
`@Composable` 어노테이션은 함수에 몇 가지 중요한 속성을 갖게하여 Compose 런타임이 다양한 최적화를 수행할 수 있게 합니다.
...
위 가정을 기반으로 Compose 런타임은 재구성과 같은 작업을 수행합니다.
