...
또한, 컴포저블 내에서 실행되는 이펙트는 라이프사이클을 가지며, 여러 번의 Recomposition 동안 작업을 지속할 수 있습니다.
...
만약 호출이 허용된다면, 호출 시 슬롯 테이블에 불필요한 공간을 차지하게 되며, 첫 번재 Composition 이후에는 이 람다가 더 이상 호출되지 않기 때문에 슬롯이 폐기될 것입니다.
...
이 그룹은 런타임에 발행되어 컴포지션이 Recomposition 시, 데이터를 대체하거나 이동할 때 컴포저블의 아이덴티티를 유지하면서 적절하게 처리할 수 있도록 정보를 제공합니다.  
...
이 어노테이션이 추가되면, 컴파일러는 해당 함수가 Recomposition 중 재실행되거나 스킵될 수 있도록 하는 보일러플레이트 코드를 생성하지 않습니다.   
이 어노테이션은 재시작되지 않더라도 큰 영향이 없는 매우 작은 함수에만 드물게 사용되어야 하며, 
이러한 함수는 부모 컴포저블의 Recomposition에 의해 자연스럽게 갱신될 가능성이 높기 때문에 스스로 무효화될 필요가 없습니다.   
...
초기화 이후 값이 절대 변경되지 않는다는 가정하에, 런타임은 '스마트 Recomposition'과 'Recomposition 스킵' 기능을 최적화할 수 있습니다.
...
> Compose 컴파일러는 두 어노테이션을 동일하게 취급하여 스마트 Recomposition 및 Recomposition 스킵을 활성화하고 최적화합니다.  
...
- 컴포저블 본문을 래핑하여 다음과 같은 작업을 수행합니다:
  ...
  - 함수가 불필요한 Recomposition을 스킵할 수 있도록 처리합니다.
  - 상태 변경에 대한 정보를 트리 하위로 전달하여, 상태 변경 시 자동으로 Recomposition될 수 있도록 합니다.

이제 Compose 컴파일러 플러그인이 적용하는 모든 종류의 Lowering에 대해 알아보겠습니다.

## Inferring class stability

스마트 Recomposition은 컴포저블의 입력값이 변하지 않고 **해당 입력값이 안정적일 때** Recomposition을 스킵하는 기능입니다.  
이 과정에서 안정성은 매우 중요한 개념이며, 이는 Compose 런타임이 안전하게 입력값을 비교하여 필요 시 Recomposition을 스킵할 수 있게 합니다.   
...
- 해당 타입의 public 프로퍼티가 변경될 때마다 컴포지션이 알림을 받아야 합니다.  
  ...
  - 이처럼 상태 간 비동기화가 발생하면, 항상 Recomposition을 트리거되어야 하며, 스마트 Recomposition은 이 입력에 의존할 수 없습니다.
...
대표적으로 `MutableState`가 있으며, 이는 상태가 변경될 때마다 Compose에 의해 감지되기 때문에, 스마트 Recomposition에 안전하게 사용할 수 있는 타입입니다.
...
이를 통해, Compose는 클래스에 의존하는 컴포저블들이 Recomposition이 필요한지 여부를 판단할 수 있습니다.
...
컴포저블에 전달된 람다를 자동으로 메모이제이션하면, Recomposition이 발생할 때 람다의 재사용성을 높일 수 있어 성능 최적화에 도움이 됩니다.
...
그 중 하나는 `$changed` 파라미터로, 현재 컴포저블의 입력값이 이전 Composition 이후 변경되었는지에 대한 단서를 제공합니다.  
이를 통해, 입력값이 변경되지 않은 경우 불필요한 Recomposition을 건너뛸 수 있습니다.
...
- 입력 파라미터가 이전 Composition 이후로 변경되지 않았거나, 변경되었더라도 이미 부모 컴포저블에서 비교가 완료된 경우, 다시 비교할 필요가 없습니다.  
...
값이 "dirty" 상태, 즉 변경되었다면 함수 본문이 호출(Recomposition)되고, 그렇지 않으면 컴포저블은 Recomposition을 스킵합니다.

Recomposition은 여러 번 일어날 수 있기 때문에, 입력 상태의 변화를 추적하는 것이 중요한데, 이는 불필요한 연산 시간과 메모리 공간을 절약할 수 있습니다.  
...
이를 "비교 전파(comparison propagation)"라고 부르며, 이 정보는 Composition 동안 본문에서 사용할 수 있습니다.  
...
이 덕분에 함수가 `List<T>`와 같은 광범위한 타입을 받아들이더라도, 안정적인 것으로 추론되면(e.g : `listOf(1, 2)`), Recomposition을 스킵할 수 있습니다.
...
Compose 컴파일러는 컴포저블 본문에 **그룹**을 삽입하며, 본문의 제어 흐름 구조에 따라 다양한 그룹을 생성할 수 있습니다:

- Replaceable 그룹 
- Movable 그룹
- Restartable 그룹

컴포저블은 런타임에 그룹을 발행하며, 이 그룹들은 컴포저블 호출과 관련된 모든 상태 정보를 포함합니다.  
이러한 그룹 덕분에 컴포지션은 그룹을 교체해야 할 때 작성된 데이터를 정리하거나(replaceable 그룹), 컴포저블의 아이덴티티를 유지하면서 이동시키거나(movable 그룹), Recomposition 중 함수를 다시 시작할 수 있습니다(restartable 그룹).

결국 런타임은 컴포지션이 메모리에 저장한 정보를 바탕으로 제어 흐름을 어떻게 처리할지 알아야 합니다.

그룹은 호출 위치에 대한 정보를 포함하고 있으며, 호출이 소스 코드 내에서 차지하는 텍스트 범위를 래핑합니다.  
호출 위치를 기반으로 키가 생성되어 그룹이 저장되며, 이를 통해 위치 기반 메모이제이션이 활성화됩니다.

### Replaceable groups

이전에 설명했듯이, 컴포저블 람다의 본문은 자동으로 래핑됩니다.  
이 과정에서 `$composer`, 생성된 `$key`, 실제 컴포저블 람다 표현식 등의 정보가 전달받는 컴포저블 팩토리 함수 호출이 추가됩니다.

컴포저블 팩토리 함수는 다음과 같습니다:
...
이 팩토리 함수는 컴포저블의 본문으로 사용되는 람다 표현식들에 대해 호출됩니다.  
팩토리 함수를 살펴보면, 먼저 키를 사용해 Replaceable 그룹을 시작하고, 끝에서는 그룹을 닫으며 중간의 텍스트 범위를 래핑합니다.  
그룹의 시작과 종료 사이에서는 컴포지션이 관련 정보를 업데이트하며, 이 경우엔 람다 표현식 자체(`ComposableLambda { ... }`)를 업데이트합니다.

위 내용은 컴포저블 람다에 대한 설명이지만, 다른 컴포저블 호출에서도 동일하게 작동합니다.  
다음은 `NonRestartableComposable`로 표시된 일반적인 컴포저블이 어떻게 변환되는지에 대한 예시입니다:
...
컴포저블 호출은 컴포지션에 Replaceable 그룹을 발행하여 저장합니다.  

그룹은 트리처럼 구성되며, 각 그룹은 여러 자식 그룹을 포함할 수 있습니다.  
예를 들어, `Wat`도 컴포저블이라면, 컴파일러는 `Wat`에 대한 그룹을 생성하여 삽입합니다. 

아래 예시는 이전에, 컴포저블 호출의 위치를 기반으로 아이덴티티를 어떻게 유지할 수 있는지를 보여주는 예제로, 
런타임이 아래와 같은 `Text` 호출 두 개를 서로 다르게 인식할 수 있습니다:
...
위와 같은 조건부 로직을 포함하는 컴포저블은 Replaceable 그룹을 발행하며, 해당 그룹은 조건이 변경될 때 교체할 수 있도록 컴포지션에 저장됩니다.

### Movable groups

Movable 그룹은 아이덴티티를 잃지 않고 재정렬할 수 있으며, 현재는 `key` 호출의 본문에서만 필요합니다.  
...
`key` 컴포저블로 `Talk`를 래핑하는 것은 해당 요소에 예외 없이 고유한 아이덴티티가 부여하는 방법입니다.   
이처럼 컴포저블을 `key`로 래핑하면 Movable 그룹이 생성되며, 이를 통해 아이템들이 정렬 순서가 변경되더라도 아이덴티티를 유지할 수 있습니다.
...
Restartable 그룹은 가장 흥미로운 그룹 중 하나로, 재시작 가능한 컴포저블에만 삽입됩니다.  
Restartable 그룹은 컴포저블 호출을 래핑하고, `end` 호출을 확장하여 nullable 값을 반환하도록 만듭니다.  
`end` 호출이 `null` 값을 반환하면, 컴포저블 본문이 변경될 수 있는 상태를 읽지 않기 때문에 **Recomposition이 필요하지 않습니다.**  
따라서, 런타임에서는 해당 컴포저블을 어떻게 Recomposition해야 하는지를 가르칠 필요가 없습니다.  
반면, non-null이 반환되면, 컴파일러는 런타임에서 해당 컴포저블을 "재시작"(다시 실행)하고 컴포지션을 업데이트 할 수 있도록 하는 람다를 생성합니다.
...
Recomposition을 위한 `updateScope` 내에서 동일한 컴포저블을 새롭게 호출하는 것을 볼 수 있습니다.  
이는 상태를 읽는 모든 컴포저블은 Recomposition 시, 특정 타입(Restartable)의 그룹을 생성함을 의미합니다.

이 섹션을 마무리하기 전에, 컴파일러가 서로 다른 타입의 그룹을 생성하기 위해, 실행 가능한 블록에서 적용하는 추가적인 논리를 설명하겠습니다. 

- 블록이 항상 한 번만 실행된다면, 별도의 그룹이 필요하지 않습니다.
- 여러 블록 중 단 하나만 실행되는 경우(e.g : `if`, `when` 절의 결과 블록), 각 블록에 Replaceable 그룹을 삽입합니다. 이는 조건부 논리로 처리됩니다.
- Movable 그룹은 오직 `key` 호출의 콘텐츠 람다에서만 필요합니다.