...
이러한 그룹 덕분에 컴포지션은 그룹을 교체해야 할 때 작성된 데이터를 정리하거나(replaceable 그룹), 컴포저블의 아이덴티티를 유지하면서 이동시키거나(movable 그룹), 리컴포지션 중 함수를 다시 시작할 수 있습니다(restartable 그룹).
...
`end` 호출이 `null` 값을 반환하면, 컴포저블 본문이 변경될 수 있는 상태를 읽지 않기 때문에 **리컴포지션이 필요하지 않습니다.**  
따라서, 런타임에서는 해당 컴포저블을 어떻게 리컴포지션해야 하는지를 가르칠 필요가 없습니다.  
...
리컴포지션을 위한 `updateScope` 내에서 동일한 컴포저블을 새롭게 호출하는 것을 볼 수 있습니다.  
이는 상태를 읽는 모든 컴포저블은 리컴포지션 시, 특정 타입(Restartable)의 그룹을 생성함을 의미합니다.
