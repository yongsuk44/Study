...
"발행"은 먼저 슬롯 테이블을 업데이트하고, 그 후에 실체화된 트리를 업데이트 하기 위한 **지연된(deferred) 변경 사항**을 생성하는 것을 의미합니다.  
'지연된 변경 사항'들은 목록으로 저장되며, 이 새로운 변경 목록은 이미 슬롯 테이블에 저장된 현재 상태를 기반으로 만들어집니다.  
...
즉, 컴포저블이 발행될 때마다 슬롯 테이블을 참조하여, 필요한 정보에 따라 '지연된 변경 사항'을 생성하고, 이를 변경 목록에 추가합니다.  
이후 컴포지션이 완료되면, 실체화가 이루어지고, 이때 **기록된** 변경 사항들이 실제로 실행됩니다. 이 괴정에서 슬롯 테이블은 컴포지션의 최신 정보로 업데이트됩니다.   
이러한 과정 덕분에 발행 프로세스는 나중에 실행될 지연된 작업만 생성되기에 매우 빠르게 처리됩니다.

이 과정을 통해 변경 목록이 슬롯 테이블에 변경 사항을 반영하는 역할을 담당한다는 것을 알 수 있습니다.  
그 후, `Applier`에게 실체화된 노드 트리를 업데이트하도록 알림을 보냅니다.
