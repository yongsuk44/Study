Compose에서 컴포저블은 실제로 UI를 생성하는 것이 아닌, Composer를 통해 런타임이 관리하는 인메모리 구조(슬롯 테이블)에 변경 사항을 "발행"함을 알아야 합니다.  
이와 같은 표현을 나중에 해석하여 UI를 "실체화" 해야 합니다.

이처럼, 컴포저블들이 컴포지션에 변경 사항을 "발행"하고, 발행된 변경 사항을 통해 컴포지션을 업데이트할 수 있습니다.  
...
> Chapter2에서 컴파일러가 컴포저블 함수의 본문을 래핑하여 그룹을 발행하는 방법에 대해 배웠습니다.  
...
컴포지션 과정이 완료되면, 모든 컴포저블 호출이 종료되고, 변경 사항이 모두 기록됩니다.  
...
Composer는 컴포지션 과정에서 그룹을 "시작"하고 "종료"할 수 있으며, 이 동작은 현재 수행 중인 작업에 따라 다르게 해석됩니다.    
...
그리고 최종적으로 이러한 변경 사항이 어떻게 적용되는지에 대해 살펴보았습니다. 그러나, 아직 컴포지션 과정 자체에 대해서는 다루지 않았습니다.  
...
- 컴포저블이 정의되었더라도, 실제로 이를 실행하여 화면에 표시하려면 컴포지션 과정이 필요합니다.  
