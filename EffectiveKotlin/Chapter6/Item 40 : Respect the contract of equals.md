...
이러한 동작은 추가적인 ㅊ프로퍼티들이 실제로 불필요하고, 해당 프로퍼티들 없이도 객체가 정상적으로 동작할 수 있을 때만 적절하다.
...
- Consistent : 'null'이 아닌 어떤 값 'x'와 'y'에 대해, 'equals' 비교에 사용되는 정보에 변동이 업다면, 'x.equals(y)'는 여러 번 호출되어도 일관되게 'true'를 반환하거나 'false'를 반환해야 한다.
