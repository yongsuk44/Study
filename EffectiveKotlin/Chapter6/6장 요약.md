...
여러 데이터를 묶어서 전달해야하는 상황에서는 'data class'를 사용하는 것이 좋으며, 다음과 같은 유용한 함수들을 자동으로 지원한다.
...
'tuple'은 'Serializable'을 구현하고, 커스텀된 'toString'을 갖는 'generic data class' 이다.
...
하지만, 이 역시 '함수 타입'은 'typealias'를 통해 함수 타입에 이름을 부여할 수 있으며, 파라미터에 이름을 지정하면 IDE에서 기본적으로 제안 받을 수 있다.
...
또한, 람다 표현식은 코드 내 인자들을 간편하게 'destructuring' 할 수 있어, 각 인자를 명확하고 간결하게 다룰 수 있따.
